...
It currently uses Julia 1.6.0-rc2.
...
if (!(Test-Path -Path "$cache\julia.zip")) {
    ...
    Invoke-WebRequest -Uri "https://julialang-s3.julialang.org/bin/winnt/x64/1.6/julia-1.6.0-rc2-win64.zip" -OutFile "$cache\julia.zip"
}
...
Move-Item -Path "$mount\julia-1.6.0-rc2" -Destination "$mount\julia"
